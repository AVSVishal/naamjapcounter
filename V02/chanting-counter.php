...
html{font-size:16px}
body{font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);min-height:100vh;min-height:-webkit-fill-available;display:flex;align-items:center;justify-content:center;padding:1rem;user-select:none;-webkit-tap-highlight-color:transparent;overflow-x:hidden}
...
.count-display{font-size:clamp(3.5rem,15vw,6rem);font-weight:700;color:#667eea;margin:clamp(1rem,4vw,2rem) 0;font-variant-numeric:tabular-nums;letter-spacing:-0.05em;line-height:1;word-break:break-all}
.btn-count{width:clamp(140px,40vw,220px);height:clamp(140px,40vw,220px);max-width:90vw;max-height:90vw;border-radius:50%;border:none;background:linear-gradient(135deg,#ff6b35,#f7931e);color:#fff;font-size:clamp(1.25rem,5vw,2.5rem);font-weight:700;cursor:pointer;box-shadow:0 10px 30px rgba(255,107,53,.4);transition:transform .1s,box-shadow .1s;touch-action:manipulation;margin:1rem auto;display:flex;align-items:center;justify-content:center}
